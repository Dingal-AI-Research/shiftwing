...
#include "quant.h"
#include "tok.h"
#include "glm53_compat.h"   /* st_read_f32_cap, tok_free, coli_serve_binary_mode */
#ifdef COLI_VULKAN
...
typedef struct {
    GLayerState *layer;
    float *kda_scratch;
    /* Lo stato nascosto dell'ultima posizione, prima della norma finale: e'
     * l'ingresso che la testa MTP fonde con l'embedding del token successivo.
     * Si tiene qui perche' appartiene alla conversazione, non al passaggio. */
    float *last_hidden;
...
typedef struct {
    ...
    long hits, miss;
    int direct_experts;                   /* leggi gli esperti con O_DIRECT */
    int uring_experts;                    /* una sola sottomissione per blocco */
    /* Il layer MTP vive dopo num_hidden_layers. Si carica solo se lo si usa:
     * porta i suoi 288 esperti e le sue norme, e chi decodifica senza
     * speculazione non ha motivo di pagarli. n_held e' quindi il numero di
     * layer davvero presenti in m->layer, che non e' sempre n_layers. */
    int mtp_on;
    int n_held;
    /* Pesi propri del blocco MTP. Vivono sul modello e non sul layer: ce n'e'
     * uno solo, e la struct Layer che li dichiarava non e' quella che il
     * motore usa a runtime. */
    GlmUring uring;                       /* ring e scratch riusati fra i blocchi */
    Mat mtp_eh;
    const float *mtp_enorm, *mtp_hnorm, *mtp_head_norm;
    /* Quante bozze la testa MTP ha proposto e quante coincidevano col token
     * che il modello ha poi scelto. E' il numero da cui dipende tutto: sotto
     * il 90% la speculazione qui costa piu' di quanto rende, perche' lo stato
     * ricorrente KDA va salvato e ripristinato. */
    long mtp_proposed, mtp_accepted;
    /* Quante volte ogni esperto e' stato scelto, per layer. Serve a decidere
     * se una cache piu' furba puo' servire a qualcosa: le tecniche che
     * predicono o pinnano gli esperti caldi (FlashMoE, ProMoE, MoE-Infinity)
     * vivono tutte sulla localita', e se il routing e' piatto non c'e' niente
     * da predire.
     *
     * Misurato sul modello vero, solo decode: i primi 23 esperti di ogni layer
     * -- quanti ne tiene la cache -- coprono il 51.0% degli usi, i primi 96 il
     * 89.8%. Il routing e' molto sbilanciato, e una cache LRU ne prendeva il
     * 6.3%: buttava via gli esperti caldi appena smettevano di essere recenti.
     * Quindi il conteggio non e' piu' solo diagnostica, e' quello che decide
     * chi resta. */
    long *euse;                           /* tutti gli usi: guida lo sfratto */
    long *ehist;                          /* solo decode: solo per il rapporto */
...
static void expert_table_init(GModel *m) {
    ...
    m->eref = calloc((size_t)m->n_held * c->n_experts, sizeof(*m->eref));
    ...
    const int upto = m->layer_end + (m->mtp_on ? 1 : 0);
...
static void expert_cache_init(GModel *m) {
    const Cfg *c = &m->c;
    /* O_DIRECT per gli esperti, salvo richiesta contraria. Sul disco di questa
     * macchina il buffered legge 15.9 MB/s contro 408 MB/s in O_DIRECT quando
     * la page cache e' piena, e la page cache e' sempre piena: contiene questo
     * stesso modello. Chi ha un filesystem che rifiuta O_DIRECT ricade sul
     * buffered da solo, senza bisogno di toccare questa variabile. */
    const char *direct = getenv("GLM53_DIRECT");
    m->direct_experts = direct ? atoi(direct) != 0 : 1;
    /* Una sola sottomissione per blocco invece di una pread per esperto. Serve
     * O_DIRECT: il batch legge dai descrittori gemelli. Se il kernel non ha
     * io_uring la sottomissione fallisce e si torna al ciclo parallelo, che e'
     * una condizione normale e non un errore. */
    /* Spento salvo richiesta esplicita. Misurato su tre tentativi: 593.0 s con
     * un ring per blocco, 494.5 s con ring persistente, 438.2 s con anche la
     * copia in parallelo, contro 436.7 s del percorso per esperto che
     * sostituiva. Cioe' un pareggio, dopo aver tolto tutto quello che il
     * batch aveva aggiunto. Non c'e' niente da vincere: il decode gia' legge
     * a 2.55 GB/s su un disco che ne fa 2.6, e una diversa schedulazione non
     * crea banda che non c'e'. Resta accessibile con GLM53_URING=1 perche' su
     * un disco piu' veloce la conclusione potrebbe cambiare. */
    const char *uring = getenv("GLM53_URING");
    m->uring_experts = m->direct_experts && uring && atoi(uring) != 0;
    ...
    const int from = c->first_dense > m->layer_begin ? c->first_dense : m->layer_begin;
    /* Il layer MTP porta i suoi 288 esperti e vuole la sua quota di cache:
     * dividere il budget per 42 invece di 43 lo lasciava fuori dal conto. */
    int sparse = m->layer_end + (m->mtp_on ? 1 : 0) - from;
    ...
    m->ecache = calloc((size_t)m->n_held, sizeof(*m->ecache));
    if (!m->ecache) { fprintf(stderr, "OOM sulla cache degli esperti\n"); exit(1); }
    for (int i = from; i < m->layer_end + (m->mtp_on ? 1 : 0); i++) {
...
static void expert_read(GModel *m, int layer, int eid, Slot *slot) {
    ...
    /* O_DIRECT first. This is the hottest read in the engine -- about 4.5 GB
     * per generated token, since every token wants 8 experts on each of the 42
     * sparse layers -- and the buffered path goes through a page cache that is
     * already full of this same model. Measured on this host's shards with the
     * engine resident: 408 MB/s direct against 15.9 MB/s buffered. Falls back
     * to the buffered read, which is what a filesystem without O_DIRECT gets.
     * GLM53_DIRECT=0 restores the old behaviour for comparison. */
    if (ref->contig) {
        if (!m->direct_experts ||
            st_pread_direct_raw(&m->S, ref->fd[0], ref->off[0], m->e_slot,
                                slot->base) != 0)
            st_pread_full(ref->fd[0], slot->base, m->e_slot, ref->off[0], "expert");
    } else {
        for (int p = 0; p < GLM53_EXPERT_PIECES; p++)
            if (!m->direct_experts ||
                st_pread_direct_raw(&m->S, ref->fd[p], ref->off[p], m->e_len[p],
                                    slot->base + m->e_at[p]) != 0)
...
/* Un blocco di esperti con una sola sottomissione io_uring.
 *
 * Il ciclo qui sotto leggeva un esperto per volta dentro a un parallel for:
 * ogni thread faceva la sua pread e aspettava. Le letture sono comunque tutte
 * note in anticipo -- l'unione del blocco e' gia' stata calcolata -- quindi si
 * possono consegnare al kernel insieme e aspettare una volta sola.
 *
 * Ritorna 1 se il blocco e' stato letto, 0 se il chiamante deve rifarlo per
 * conto suo. Uno zero non lascia dati sbagliati in giro: gli slot hanno gia'
 * eid = -1, e chi ricade sul percorso vecchio li riempie da capo. */
static int expert_read_batch(GModel *m, int layer, const int *eids,
                             const int *to_read, int reads, const int *slot_of,
                             LCache *cache) {
    if (reads <= 0) return 1;
    int fds[GLM_BATCH_MAX];
    int64_t offs[GLM_BATCH_MAX], lens[GLM_BATCH_MAX];
    void *outs[GLM_BATCH_MAX];
    int n = 0;
    ...
    for (int r = 0; r < reads; r++) {
        ...
        const ERef *ref = &m->eref[(size_t)layer * m->c.n_experts + eids[i]];
        if (ref->contig) {
            if (n >= GLM_BATCH_MAX) return 0;
            fds[n] = ref->fd[0]; offs[n] = ref->off[0];
            lens[n] = m->e_slot;  outs[n] = slot->base; n++;
        } else {
            if (n + GLM53_EXPERT_PIECES > GLM_BATCH_MAX) return 0;
            for (int p = 0; p < GLM53_EXPERT_PIECES; p++) {
                fds[n] = ref->fd[p]; offs[n] = ref->off[p];
                lens[n] = m->e_len[p]; outs[n] = slot->base + m->e_at[p]; n++;
            }
        }
    }
    if (glm_read_batch_uring(&m->S, &m->uring, fds, offs, lens, outs, n) != 0) return 0;

    for (int r = 0; r < reads; r++) cache->s[slot_of[to_read[r]]].eid = eids[to_read[r]];
    /* Seriale: il ciclo a blocchi non gira dentro a una regione parallela. */
    m->miss += reads;
    m->ebytes += (uint64_t)reads * (uint64_t)m->e_slot;
    return 1;
}
...
static void ffn_layer(GModel *m, const GLayer *l, int index, const float *x,
    ...
    /* Il conteggio e' sulle scelte, non sull'unione: due token che vogliono lo
     * stesso esperto sono due usi, ed e' quello che una cache vedrebbe. */
    /* Il prefill conta come il decode: e' li' che si impara chi e' caldo, e
     * arrivare al primo token generato con i contatori a zero vorrebbe dire
     * sfrattare a caso proprio quando serve di piu'. GLM53_EXPERT_HIST tiene
     * un secondo conteggio ristretto al decode, per il rapporto. */
    {
        long *use = &m->euse[(size_t)index * m->c.n_experts];
        for (int i = 0; i < tokens * topk; i++) use[chosen[i]]++;
        /* Il rapporto guarda solo il decode -- il prefill tocca tutto per
         * costruzione e appiattirebbe la curva -- ma lo sfratto guarda tutto. */
        if (m->ehist && tokens == 1) {
            long *h = &m->ehist[(size_t)index * m->c.n_experts];
            for (int i = 0; i < tokens * topk; i++) h[chosen[i]]++;
        }
    }
    ...
    /* Chi resta lo decide la frequenza, non la recenza.
     *
     * Misurato sul modello vero, solo decode: i primi 23 esperti di un layer
     * coprono il 51.0% degli usi, e la cache ne tiene esattamente 23. Una
     * cache perfetta prenderebbe quindi il 51%; LRU ne prendeva il 6.3%,
     * perche' un esperto caldo che per due token non viene scelto diventa il
     * meno recente e se ne va, per essere riletto subito dopo. Con i contatori
     * gli sfratti cadono sulla coda fredda, che e' lunga: il 67% degli usi sta
     * nei primi 46 esperti su 288.
     *
    ...
    for (int base = 0; base < n_union; base += block) {
         ...
         * sola -- assegnare uno slot lo rendeva il piu' recente, quindi
         * l'ultimo a essere scelto -- e passando alla frequenza quella
         * proprieta' si perde, quindi la finestra e' esplicita. Presa una
         * volta sola per tutto il layer, il secondo blocco trovava tutti gli
         * slot prenotati dal primo e non aveva dove andare. */
        const uint64_t block_start = m->clock;
        int reads = 0;
        for (int i = 0; i < here; i++) {
            ...
            if (cache->n < cache->cap) victim = &cache->s[cache->n++];
                ...
                int pick = -1; long coldest = 0;
                for (int j = 0; j < cache->n; j++) {
                    if (cache->s[j].used > block_start) continue;   /* prenotato */
                    long u = cache->s[j].eid >= 0 ? use[cache->s[j].eid] : -1;
                    if (pick < 0 || u < coldest ||
                        (u == coldest && cache->s[j].used < cache->s[pick].used)) {
                        coldest = u; pick = j;
                    }
                }
                /* Irraggiungibile: il blocco e' grande al piu' quanto la
                 * cache, quindi c'e' sempre uno slot non prenotato. Se mai
                 * accadesse, l'alternativa sarebbe scrivere due esperti sullo
                 * stesso slot e restituire numeri sbagliati senza dirlo. */
                if (pick < 0) {
                    fprintf(stderr, "layer %d: nessuno slot libero fra %d "
                            "(blocco %d, cap %d)\n", index, cache->n, here, cache->cap);
                    exit(1);
                }
                victim = &cache->s[pick];
            }
        ...
        int batched = m->uring_experts &&
                      expert_read_batch(m, index, union_ids + base, to_read,
                                        reads, slot_of, cache);
        if (!batched) {
#ifdef _OPENMP
...
static void model_load_range(GModel *m, const char *dir, int layer_begin,
    ...
    {   /* Default off while the speculative loop is being brought up: with it
         * off the engine must behave exactly as before, which is the control
         * the losslessness test compares against. */
        const char *want = getenv("GLM53_MTP");
        m->mtp_on = m->c.n_mtp > 0 && want && atoi(want) != 0;
    }
    m->n_held = m->c.n_layers + (m->mtp_on ? 1 : 0);
    /* Sempre allocato: serve alla politica di sfratto, non solo al rapporto. */
    m->euse = calloc((size_t)m->n_held * m->c.n_experts, sizeof(*m->euse));
    if (!m->euse) { fprintf(stderr, "OOM sui contatori degli esperti\n"); exit(1); }
    if (getenv("GLM53_EXPERT_HIST"))
        m->ehist = calloc((size_t)m->n_held * m->c.n_experts, sizeof(*m->ehist));
    m->layer = calloc((size_t)m->n_held, sizeof(*m->layer));
    ...
    for (int i = layer_begin; i < layer_end + (m->mtp_on ? 1 : 0); i++) {
        GLayer *l = &m->layer[i];
        /* Il layer MTP e' un blocco normale: niente mHC, ma le due norme di
         * ingresso e la proiezione che fonde embedding e stato nascosto. La
         * funzione che elenca i nomi (sopra) dice la stessa cosa, ed e' la
         * definizione a cui questo si attiene. */
        const int is_mtp = i >= m->c.n_layers;
        ...
        if (is_mtp) {
            m->mtp_enorm = load_f32(m, "%slayers.%d.enorm.weight", P, i);
            m->mtp_hnorm = load_f32(m, "%slayers.%d.hnorm.weight", P, i);
            m->mtp_head_norm = load_f32(m, "%slayers.%d.shared_head.norm.weight", P, i);
            m->mtp_eh = load_mat(m, "%slayers.%d.eh_proj.weight", P, i);
        } else {
...
static GSession *session_open(const GModel *m, int cap) {
    ...
    s->cap = cap;
    s->n_layers = m->n_held;
    s->layer = calloc((size_t)s->n_layers, sizeof(*s->layer));
    if (!s->layer) { fprintf(stderr, "OOM sugli stati di layer\n"); exit(1); }
    if (m->mtp_on) {
        s->last_hidden = calloc((size_t)c->hidden, sizeof(float));
        if (!s->last_hidden) { fprintf(stderr, "OOM sullo stato MTP\n"); exit(1); }
    }
    if (c->kda_proj)
...
static void session_close(const GModel *m, GSession *s) {
    if (!s) return;
    (void)m;   /* la sessione sa da sola quanti stati ha */
    for (int i = 0; i < s->n_layers; i++) {
        GLayerState *st = &s->layer[i];
...
static void model_release(GModel *m) {
    if (!m) return;
    if (m->layer) {
        /* n_held, non layer_end: il blocco MTP sta dopo ed e' comunque nostro. */
        for (int i = m->layer_begin; i < m->n_held; i++) {
            GLayer *l = &m->layer[i];
    ...
    free(m->ehist);
    free(m->eref);
    mat_release(&m->mtp_eh);
    free((void *)m->mtp_enorm); free((void *)m->mtp_hnorm);
    free((void *)m->mtp_head_norm);
    if (m->vblocks) {
...
/* Un passo della testa MTP: dato lo stato nascosto dell'ultima posizione e il
 * token che la segue, propone il token dopo ancora.
 *
 * Il blocco MTP non ha le iper-connessioni degli altri 45 layer -- il
 * checkpoint non porta i suoi tensori hc_* e la funzione che elenca i nomi lo
 * dice esplicitamente -- quindi qui il residuo e' quello classico, sommato una
 * volta per sito, invece del collasso pesato su hc_mult flussi.
 *
 * Non tocca s->filled: la posizione la decide il chiamante, e una bozza
 * rifiutata deve poter essere riscritta dalla successiva. */
static int mtp_step(GModel *m, GSession *s, int token, int pos, float *logits) {
    const Cfg *c = &m->c;
    const int D = c->hidden, L = c->n_layers;
    if (!m->mtp_on || !s->last_hidden) return -1;
    if (token < 0 || token >= c->vocab) return -1;

    GLayer *l = &m->layer[L];
    float *cat = malloc((size_t)2 * D * sizeof(float));
    float *x = malloc((size_t)D * sizeof(float));
    float *normed = malloc((size_t)D * sizeof(float));
    float *branch = malloc((size_t)D * sizeof(float));
    if (!cat || !x || !normed || !branch) { fprintf(stderr, "OOM nel passo MTP\n"); exit(1); }

    /* [norma(embedding del token) ; norma(stato nascosto)] -> eh_proj */
    rms(cat, m->embed + (size_t)token * D, m->mtp_enorm, D, c->eps);
    rms(cat + D, s->last_hidden, m->mtp_hnorm, D, c->eps);
    mv(x, &m->mtp_eh, cat);

    GLayerState *st = &s->layer[L];
    rms(normed, x, l->in_ln, D, c->eps);
    mla_layer(c, l, normed, 1, branch, st, pos);
    for (int d = 0; d < D; d++) x[d] += branch[d];
    rms(normed, x, l->post_ln, D, c->eps);
    ffn_layer(m, l, L, normed, 1, branch);
    for (int d = 0; d < D; d++) x[d] += branch[d];

    rms(normed, x, m->mtp_head_norm, D, c->eps);
    mv(logits, &m->head, normed);

    int best = 0;
    for (int v = 1; v < c->vocab; v++) if (logits[v] > logits[best]) best = v;
    free(cat); free(x); free(normed); free(branch);
    return best;
}
...
static void serve_one(GModel *m, Tok *tokenizer, ServeReq *q) {
    ...
    if (getenv("GLM53_VERBOSE"))
        fprintf(stderr, "REUSE %llu %d %d uring-batches %llu reads %llu fallbacks %llu\n",
...
int main(int argc, char **argv) {
    ...
    if (greedy > 0) {
        ...
        float *draft_logits = model.mtp_on
            ? malloc((size_t)model.c.vocab * sizeof(float)) : NULL;
        int pending_draft = -1;
        for (int step = 0; step < greedy; step++) {
            int next = argmax(logits + (size_t)(rows - 1) * model.c.vocab, model.c.vocab);
            /* Modalita' osservazione: la bozza dell'iterazione precedente si
             * confronta con quello che il modello ha davvero scelto, e non
             * cambia nulla di cio' che esce. Serve a misurare l'accettazione
             * prima di farci affidamento. */
            if (pending_draft >= 0) {
                model.mtp_proposed++;
                if (pending_draft == next) model.mtp_accepted++;
            }
            free(logits);
            logits = NULL;
            int done = 0;
            ...
            /* La bozza si forma prima del passaggio: usa lo stato nascosto
             * dell'ultima posizione, che il passaggio sta per sostituire. */
            if (draft_logits)
                pending_draft = mtp_step(&model, session, next, session->filled,
                                         draft_logits);
        ...
        if (model.mtp_proposed)
            printf("mtp proposte %ld accettate %ld = %.1f%%\n",
                   model.mtp_proposed, model.mtp_accepted,
                   100.0 * model.mtp_accepted / (double)model.mtp_proposed);
    }
    ...
    if (model.ehist) {
        /* Quanto renderebbe una cache perfetta, cioe' il tetto di qualsiasi
         * politica basata sulla localita': si ordinano gli esperti per uso e
         * si guarda che quota di usi coprono i primi N. Se la curva e' piatta
         * non c'e' niente che una cache piu' furba possa recuperare, e le
         * tecniche che predicono gli esperti caldi non hanno appiglio. */
        const int E = model.c.n_experts;
        const int caps[] = { 8, 23, 46, 96 };
        double share[4] = {0}, total = 0;
        int layers_counted = 0;
        long *row = malloc((size_t)E * sizeof(long));
        for (int i = 0; i < model.n_held; i++) {
            long *src = &model.ehist[(size_t)i * E];
            long sum = 0;
            for (int e = 0; e < E; e++) sum += src[e];
            if (!sum) continue;
            layers_counted++;
            total += (double)sum;
            memcpy(row, src, (size_t)E * sizeof(long));
            /* ordinamento decrescente, insertion sort: E e' 288 */
            for (int a = 1; a < E; a++) {
                long v = row[a]; int b = a - 1;
                while (b >= 0 && row[b] < v) { row[b + 1] = row[b]; b--; }
                row[b + 1] = v;
            }
            for (int k = 0; k < 4; k++) {
                long top = 0;
                for (int e = 0; e < caps[k] && e < E; e++) top += row[e];
                share[k] += (double)top / (double)sum;
            }
        }
        free(row);
        if (layers_counted) {
            printf("expert use (decode): %.0f scelte su %d layer, %d esperti\n",
                   total, layers_counted, E);
            for (int k = 0; k < 4; k++)
                printf("  i primi %3d esperti (%4.1f%% della cache) coprono %5.1f%% degli usi\n",
                       caps[k], 100.0 * caps[k] / E, 100.0 * share[k] / layers_counted);
        }
    }
    if (model.streaming)
        /* uring-batches distingue "il batch ha funzionato" da "e' ricaduto sul
         * ciclo parallelo", che dall'esterno hanno lo stesso identico output. */
        printf("experts hits %ld miss %ld bytes %llu "
               "uring-batches %llu uring-reads %llu uring-fallbacks %llu\n",
               model.hits, model.miss, (unsigned long long)model.ebytes,
               (unsigned long long)model.S.uring_batches,
               (unsigned long long)model.S.uring_reads,
               (unsigned long long)model.S.uring_fallbacks);
    free(logits);
