...
def stamp_family(outdir):
    """Record the family in the converted config.json.

    openai_server.py picks its chat template from this marker and falls back to
    Qwen when it is absent. GLM-5.3's template is [gMASK]<sop> with
    <|system|>/<|user|>/<|assistant|> and shares no control token with Qwen's
    <|im_start|> form, so an unstamped snapshot is served with a prompt the
    model was never trained on -- which looks like a bad model, not a missing
    field.
    """
    ...
    try:
        ...
        print(f"[family] cannot stamp {path}: {exc}", flush=True)
        return
    config["shiftwing_model_family"] = "glm-5.3"
    with open(path, "w") as handle:
        json.dump(config, handle, indent=2)
    print("[family] shiftwing_model_family=glm-5.3", flush=True)
