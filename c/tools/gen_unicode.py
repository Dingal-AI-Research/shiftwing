"""Genera tok_unicode.h: tabelle di range per le classi Unicode dei tokenizer:
  - \\p{L}  lettere   (categoria Unicode che inizia per 'L')
  - \\p{M}  marks     (categoria Unicode che inizia per 'M')
...
# Hugging Face tokenizers' regex engine treats variation selectors as the
# optional non-letter prefix in Qwen's pattern, not as ``\p{M}``, despite their
# Unicode Mn category.  Match the observed tokenizer behavior exactly.
M = ranges(lambda c: cat(c).startswith("M") and not (0xFE00 <= c <= 0xFE0F or 0xE0100 <= c <= 0xE01EF))
...
emit("uni_L", L); emit("uni_M", M); emit("uni_N", N); emit("uni_S", S)
